...
- **Token efficiency**: be direct and concise. No filler phrases, no trailing summaries. Answer first, expand only on request.
...
When a code review is explicitly requested, invoke the `adversarial-review` skill. Do NOT auto-trigger it after every diff or implementation — it is opt-in only to save tokens.

## Deep Reasoning

Invoke the `deep-reason` skill when the user explicitly asks for deep reasoning or when the task involves critical algorithm/architecture decisions. Do NOT auto-trigger it for every non-trivial code change — it consumes 3-10x tokens. For routine implementation work, reason briefly inline and proceed.
