...
# --- Open Router fallback indicator ---
...
# Color for fallback indicator (amber warning)
C_FALLBACK="\033[38;5;214m"

# Segment 1: Model
segments+=("${ICON_MODEL} ${C_MODEL}${model}${C_R}")

# Segment 1b: Open Router fallback indicator (when active)
if $FALLBACK_ACTIVE; then
    if $_use_emoji; then
        segments+=("${C_FALLBACK}\xf0\x9f\x94\x80 OR${C_R}")
    else
        segments+=("${C_FALLBACK}[OR]${C_R}")
    fi
fi
